...
FIXED (Feb 2026):
- Wire transfer revenue now properly identified by "WIRE TSF" only
- Prevents double-counting when PWC pays on behalf of Long Run
- Mobile deposits properly flagged for review
- Branch deposits now recognized as revenue
"""
...
class TransactionClassifier:
    ...
    # FIXED: Classification rules with proper regex grouping
    # Format: (pattern, category, is_personal, needs_review)
    CLASSIFICATION_RULES = [
        # ===== REVENUE (Credits) =====
        # FIXED: Wire transfers - match "WIRE TSF" for ALL wire transfers
        # This catches Long Run, PWC, and any other client paying via wire
        # Do NOT separate by customer name to avoid double-counting
        (r'WIRE TSF', 'Revenue - Oilfield Services', False, False),
        ...
        (r'LOAN PAYMENT.*SCOTIA|SCOTIA.*LOAN', 'Loan Payment - Personal', True, False),
    ...
    def classify_transaction(self, description: str, debit: float, credit: float) -> Dict:
        ...
        Returns:
            Dict with cra_category, is_personal, needs_review, itc_amount, notes
        """
        ...
        if credit > 0:
            return {
                ...
                'needs_review': True,
                'itc_amount': 0.0,
                'notes': 'Unclassified credit - review required'
            }
        else:
            return {
                ...
                'notes': 'Unclassified expense - review required'
            }
    ...
    def classify_dataframe(self, df: pd.DataFrame, account_type: str = 'corporate') -> pd.DataFrame:
        ...
        IMPORTANT: This now removes duplicate rows to prevent double-counting.
        
        Args:
            df: DataFrame with date, description, debit, credit columns
            account_type: 'corporate' or 'personal'
            
        Returns:
            Classified DataFrame with CRA categories and ITC amounts
        """
        df = df.copy()
        
        # CRITICAL: Remove exact duplicate rows to prevent double-counting
...
class PersonalAccountClassifier:
    """
    Identifies potential business expenses in personal accounts
    ...
    def identify_business_expenses(self, df: pd.DataFrame) -> pd.DataFrame:
        """Flag potential business expenses in personal account"""
        df = df.copy()
